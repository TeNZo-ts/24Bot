...
markup_algebra = ReplyKeyboardMarkup(resize_keyboard=True).row(Kamida_T, Zialy_A).row(Murat_B, Asem_B)
markup_phys_geo = ReplyKeyboardMarkup(resize_keyboard=True).row(Maulet_K, Aigerim_E)
markup_geom = ReplyKeyboardMarkup(resize_keyboard=True).row(Kamida_T, Zialy_A).row(Murat_B, Asem_B)
...
markup_law_bas = ReplyKeyboardMarkup(resize_keyboard=True).add(answer_law_bas_10).add(answer_law_bas_11).add(answer_law_bas_12).add(answer_law_bas_13).add(answer_law_bas_14).add(answer_law_bas_15).add(answer_law_bas_16).add(answer_law_bas_17).add(answer_law_bas_18).add(answer_law_bas_19).add(answer_law_bas_20).add(answer_law_bas_21).add(answer_law_bas_22).add(answer_law_bas_23)
...
async def menu(message: types.Message):
	...
	# Info
		...
		await message.reply('Привет👋 Я бот🤖\n\nМои основатели - @TeNZo_ts, @Kezuh\n\nКоманды:\n/start - Запустить бота\n/open - Открыть меню\n/close - Закрыть меню\n\nНа данный момент доступно:⤵️\n10(a) класс: Понедельник, Вторник\n10(ае) класс: FULL\nDaryn Online\nПриятного пользования!💫', reply_markup=markup_back)
	# Lyceum
	...
	# Wednesday 
	elif message.text == 'Среда 10(a)':
		await message.reply('➡️', reply_markup=markup_wed_10a)
	# Thursday
	elif message.text == 'Четверг 10(a)':
		await message.reply('➡️', reply_markup=markup_thu_10a)
	# Friday
	elif message.text == 'Пятница 10(a)':
		await message.reply('➡️', reply_markup=markup_fri_10a)
	# Class 10ae
	...
	# Teachers
		...
		await message.answer('English\nАсель Муханова -\nПодключиться:\nhttps://us04web.zoom.us/j/8293276753?pwd=NERsMHo5aENYRDlwdnBSSklzU0JaUT09\nИдентификатор: 829 327 6753\nКод: j2JN12', reply_markup=markup_classes)
		...
		await message.answer('Алгебра\nКамида Тулеметова -\nПодключиться:\nhttps://us04web.zoom.us/j/2681244460?pwd=TWFCdTVDVU9ORUYvT05OWlo3UUU1Zz09\nИдентификатор: 268 124 4460\nКод: 144475', reply_markup=markup_classes)
	...
	# Law Bas
		...
		await message.reply('Выберите тему:', reply_markup=markup_law_bas)
